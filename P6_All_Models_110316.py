...
def turf_4e(CoName, CoGDB, FTG_LU, FEDS, FTGparcels):
...
def turf_5b(CoName, CoGDB, FTG_LU, FEDS):
...
def for_1(CoName, DEV113, TC, RURmask, URBmask):
...
def for_5(CoName, DEV18, TC, RURmask, URBmask):
...
def wet_1(CoName, CoGDB, LuGDB, fc_Tidal, cc_wetlands, nwi_Tidal):
...
def wet_2(CoName, CoGDB, LuGDB, fc_FPlain, cc_wetlands, nwi_FPlain):
...
def wet_3(CoName, CoGDB, LuGDB, fc_OTHWL, cc_wetlands, nwi_OTHWL):
...
def mo_1(CoName, CoGDB, LuGDB, TREES, SS, HERB):
    start_time = time.time()
    inrasList = [ ]
...
def main():
    ...
    # Loop through each gdb, extract name, strip "_Mask.gdb" & use it as a variable
    ...
    for gdb in arcpy.ListWorkspaces("*"):
        ...
        RURmask = os.path.join(str(CoGDB) + str(CoName) + "_URBmask")
        URBmask = os.path.join(str(CoGDB) + str(CoName) + "_RURmask")
        WAT_FOR = os.path.join(str(CoGDB),str(CoName) + "_watFOR")
        POT_FOR = os.path.join(str(CoGDB),str(CoName) + "_potFOR")
        ...
        if arcpy.Exists(PARCELS):
            ...
            turf_5b(CoName, CoGDB, FTG_LU, FEDS)
        ...
        for_1(CoName, DEV113, TC, RURmask, URBmask)

        for_2(DEV27, TC, CoName)

        for_3(CoName, CoGDB, RLTCP, EDGE)

        for_4(CoName, DEV37, CDEdge)

        for_5(CoName, DEV18, TC, RURmask, URBmask)
        ...
        if arcpy.Exists(cc_wetlands):
            ...
            wet_1(CoName, CoGDB, LuGDB, fc_Tidal, cc_wetlands, nwi_Tidal)

            wet_2(CoName, CoGDB, LuGDB, fc_FPlain, cc_wetlands, nwi_FPlain)

            wet_3(CoName, CoGDB, LuGDB, fc_OTHWL, cc_wetlands, nwi_OTHWL)
        ...
        if not inrasListMO:
            ...
            mo_1(CoName, CoGDB, LuGDB, TREES, SS, HERB)
        ...
        TempDirectory = "C:/A__P6_Analyst/A__P6_Temp/"
